...
def read_excel(file_path):
    ...
    variable_datatypes = [cell.value for cell in ws[2][1:]]

    # 检查第二行的数据类型是否正确填写
    invalid_datatypes = {}
    for idx, datatype in enumerate(variable_datatypes):
        if datatype not in UNITY_DATATYPES:
...
def generate_csv(csv_file_path, Table_data, variable_datatypes):
    ...
    with open(csv_file_path, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        # 写入参数类型列表
        writer.writerow(variable_datatypes)
...
" ╚═════╝╚══════╝  ╚═══╝  ╚═╝     ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝  ",                                  
    ...
    for filename in os.listdir(input_folder_name):
        if filename.endswith('.xlsx'):
            ...
            try:
                ...
                print("Number of entries in CSV:", len(weapon_data))
                print("Number of columns in CSV:", len(variable_datatypes))
                print("=====================================================================================================")
    ...
    if invalid_datatypes:
        ...
        for filename, warning in invalid_datatypes.items():
            ...
            for idx, datatype in enumerate(variable_datatypes):
                if datatype not in UNITY_DATATYPES:
                    print("Column {}: {} - Invalid datatype '{}' found.".format(idx + 1, weapon_data[0].keys()[idx], datatype))
            print("=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=!=")
            num_Error_files +=1
...
if __name__ == "__main__":
    ...
    # 构建输入文件夹的相对路径
    input_folder_name = os.path.normpath(os.path.join(script_dir, '../'))
