...
public class ButtonListener2CanvasGroup : MonoBehaviour
{
...
[CustomEditor(typeof(ButtonListener2CanvasGroup))]
public class ButtonListener2CanvasGroupEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (targetObjectProperty.objectReferenceValue != null)
        {
            ...
            // 显示脚本选择下拉列表
            int selectedScriptIndex = EditorGUILayout.Popup("Script", GetSelectedIndex(targetScriptProperty.objectReferenceValue.name), scriptNames);
    ...
    private void RefreshMethodNames()
    {
        ...
        ButtonListener2CanvasGroup buttonClickListener = (ButtonListener2CanvasGroup)target;
    ...
    private bool ParametersAreSupported(ParameterInfo[] parameters)
    {
        foreach (var parameter in parameters)
        {
            if (parameter.ParameterType != typeof(CanvasGroup))
            {
                return false;
            }
        }
