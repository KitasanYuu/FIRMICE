...
public class WeaponShooter : MonoBehaviour
{
    ...
    {
        // 开火
        if (starterAssetsInputs.shoot && tpsShootController.isAiming)
        {
            if (Starassetsinputs && Tpsshootcontroller)
            {
                // 获取当前时间
                ...
                if (currentTime - lastShootTime > fireRate)
                {
                    ...
                    if (bullettest != null)
                    {
                        ...
                        bullettest.SetDamage(BulletDamage);
                        bullettest.SetRayLength(bulletspeed * SRR);
    ...
    private void ComponemetInit()
    {
        ...
        if (_hasanimator)
        {
            Starassetsinputs = TryGetComponent<StarterAssetsInputs>(out starterAssetsInputs);
            Tpsshootcontroller = TryGetComponent<TPSShootController>(out tpsShootController);
        }
        else
        {
            Starassetsinputs = Shooter.TryGetComponent<StarterAssetsInputs>(out starterAssetsInputs);
            Tpsshootcontroller = Shooter.TryGetComponent<TPSShootController>(out tpsShootController);
        }

        if(Tpsshootcontroller && Starassetsinputs)
        {
